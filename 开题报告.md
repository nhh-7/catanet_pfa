...
| 导师姓名         |                                                                                                                |
| 攻读学位         |                                                                                                                |
| 论文题目（中文） | 面向轻量级图像超分辨率的内容自适应聚合与渐进聚焦重建方法                                                       |
| 论文题目（英文） | A Content-Adaptive Aggregation and Progressive Focused Reconstruction Method for Lightweight Image Super-Resolution |
| 选题来源         | 自拟选题                                                                                                       |
| 论文类型         | 基础研究                                                                                                       |
| 论文是否涉密     | 否                                                                                                             |
| 开题日期         |                                                                                                                |
...
1. 针对 CATANet 等内容感知聚合方法虽已提升跨区域相似 token 利用效率，但对动态语义中心、归属置信度以及路由元信息跨模块复用刻画仍不充分的问题，拟提出动态语义原型路由模块 DPR，研究基于图像内容自适应生成语义原型、建立 token 与原型之间的关联关系，并完成内容排序、相似区域聚合和候选集合构建的方法，以期达到在轻量级计算预算下提升远距离相似信息组织、筛选与重建协同能力的目标。
...
(1) CATANet 已经通过内容感知聚合提升了跨区域相似 token 的利用效率，但其内容组织结果与后续稀疏连接筛选、注意力状态递推和多级重建之间的耦合仍有进一步强化空间。本文拟在 CATANet 内容感知聚合思路的基础上，通过动态语义原型路由机制显式生成语义中心、刻画 token 归属置信度，并将路由元信息持续传递给 PFSA 和 LMR，以提升远距离相似信息的组织、筛选与重建协同能力。
