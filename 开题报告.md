...
单幅图像超分辨率是低层视觉领域中的经典问题，其核心目标是从低分辨率图像中恢复具有清晰结构和丰富纹理的高分辨率图像。近年来，CVPR、ICCV、ECCV 等计算机视觉顶级会议围绕高效 Transformer、内容自适应聚合、动态稀疏注意力和端侧部署等方向持续推出新方法，使超分辨率研究从单纯追求重建精度，逐步转向兼顾重建质量、长程依赖建模、推理效率和轻量化部署的综合设计。这一趋势与本文面向轻量级图像超分辨率的内容自适应聚合与渐进聚焦注意力网络研究具有直接关联。
...
本文围绕面向轻量级图像超分辨率任务的内容自适应聚合与渐进聚焦注意力网络 CAPANet 展开研究，整体目标是在参数量、计算量和推理速度受限的条件下，提高模型对复杂边缘、重复纹理和跨区域相似结构的恢复能力。与单纯堆叠卷积层或扩大注意力窗口的方法不同，本文拟从“内容组织、连接筛选、层次重建”三个角度构建统一的轻量级超分辨率框架：首先依据输入图像内容动态组织相似 token，使远距离相似纹理具备可交互的候选集合；其次结合当前层内容相关性与前序注意力先验筛选关键连接，减少无关 token 之间的冗余交互；最后按照低频结构、中频过渡和高频纹理的递进关系完成协同重建。CAPANet 的总体结构如图 4 所示。
...
其中，CAPANet Block 是本文方法的核心单元，主要由动态语义原型路由模块 DPR、渐进聚焦稀疏注意力模块 PFSA、低到高多级协同重建模块 LMR 以及轻量级前馈细化结构组成。每个 Block 内部先对输入特征进行归一化与内容路由，再基于路由结果执行多级稀疏注意力交互，随后通过投影层、前馈层和残差连接完成特征更新。为了使注意力聚焦信息能够在网络深层逐步稳定，本文拟在相邻 Block 之间传递低频、中频和高频三个层级的注意力状态，使不同尺度的结构先验在后续特征建模中持续发挥作用。

针对现有轻量级超分辨率模型难以有效组织跨区域相似纹理、长程依赖利用不足的问题，本文在 CATANet 内容感知聚合思路的基础上进行改进，设计动态语义原型路由模块 DPR，如图 5 所示。传统窗口注意力或规则分块方法通常依据空间位置划分 token，但在建筑纹理、漫画线条、重复窗格等复杂场景中，真正具有参考价值的相似结构往往分散在图像的不同位置。若仅在固定窗口内建立交互关系，模型难以充分利用这些远距离自相似信息，容易造成纹理恢复不连续、边缘结构断裂或细节重复模式不稳定等问题。
...
DPR 的核心思想是根据当前输入内容动态生成语义原型，并利用 token 与原型之间的相似性完成内容感知路由。设当前特征序列为：
...
其中 $B$ 表示批大小，$N$ 表示 token 数量，$C$ 表示通道维度。DPR 首先通过轻量映射生成一组动态语义原型：
...
其中，$\phi(\cdot)$ 和 $\psi(\cdot)$ 分别表示 token 特征与原型特征的映射函数。依据关联分数 $S$，DPR 将空间上分散但内容相近的 token 重新排序并聚合到相同或相邻的候选集合中，同时输出原始顺序恢复索引、内容分组标签、组内置信得分和动态原型特征。这样既能够打破固定窗口的空间限制，又能为后续注意力计算提供更干净、更具语义一致性的候选范围。与静态聚类或规则分组方式相比，该模块能够随输入图像内容自适应调整分组关系，使模型对新颖纹理、重复结构和非局部相似区域具有更强适应性。

为进一步提升注意力计算的有效性与资源利用率，本文设计渐进聚焦稀疏注意力模块 PFSA，如图 6 所示。现有 Transformer 超分辨率方法在组内或窗口内通常执行稠密注意力计算，默认所有 token 两两交互。该策略虽然能够增强特征表达能力，但也会引入大量低价值甚至有干扰的连接，尤其在轻量级场景下容易造成计算资源浪费，并使边界噪声或不相关纹理参与重建过程。针对这一问题，PFSA 采用“先继承、再筛选、后交互”的方式，将前序层已经形成的注意力聚焦先验与当前层内容相关性相结合，动态生成稀疏连接掩码，只保留对重建最有价值的关键依赖关系。
...
具体而言，PFSA 在每个层级维护独立的注意力状态。设 $A_{l-1}$ 为上一层或上一阶段传递的注意力先验，$A_l^{cur}$ 为当前层由内容相似性计算得到的注意力分布，则融合后的聚焦分布可表示为：

$$
\hat{A}_l = \beta A_{l-1} + (1-\beta) A_l^{cur}

$$

其中，$\beta$ 为注意力继承权重，用于控制历史聚焦先验与当前内容分布之间的平衡。在此基础上，本文进一步引入内容感知焦点率，根据路由组内部纯度和关联分数方差动态调整保留连接比例：

$$
r_l = \text{clip}(r_{base} + \lambda_p \cdot purity_l - \lambda_v \cdot var_l,\ r_{min},\ r_{max})

$$

其中，$purity_l$ 表示当前内容组的语义一致性，$var_l$ 表示组内关联分数波动程度，$r_{min}$ 与 $r_{max}$ 分别为焦点率上下界。当组内 token 语义一致性较高时，模型保留更多有效连接以增强信息传播；当组内内容较为混杂或边界复杂时，模型自动压缩交互范围，以降低无关噪声扩散。最终，PFSA 根据融合注意力分布和动态焦点率构造稀疏掩码：

$$
M_l = \text{TopK}(\hat{A}_l, r_l)
...
通过上述设计，PFSA 能够在保留关键长程依赖的同时减少无效注意力连接，使计算资源集中到更重要的结构和纹理区域，从而兼顾重建质量与推理效率。

针对现有轻量级超分辨率方法在结构连续性与高频纹理恢复之间缺少合理分工、重建过程层次性不足的问题，本文设计低到高多级协同重建模块 LMR，如图 7 所示。超分辨率重建并不是单纯的局部细节锐化过程，而是需要先恢复整体轮廓和主体结构，再逐步补足区域过渡、边缘变化和高频纹理。如果所有注意力 head 都在同一粒度上进行竞争式建模，模型容易在复杂场景中出现结构不连续、边缘错位或纹理伪影。基于这一观察，LMR 将路由后的特征显式划分为低频、中频和高频三个层级，并建立由低到高的递进式协同链路。
...
在具体实现上，LMR 对排序后的 token 按照不同尺度进行分支处理：高频分支保持原始 token 粒度，重点负责边缘、细线条和细粒度纹理恢复；中频分支通过局部 token pooling 建模区域边界与结构过渡；低频分支进一步聚合更大范围的 token，用于捕获全局轮廓和长距离结构关系。设三路特征分别为 $X^{lf}$、$X^{mf}$ 和 $X^{hf}$，则低到高递进过程可表示为：
...
其中，$\gamma_1$ 和 $\gamma_2$ 为可学习门控参数，$\text{Up}(\cdot)$ 表示将低一级特征恢复到高一级粒度的上采样或对齐操作。该过程使低频结构信息能够先引导中频过渡建模，再由中频结果约束高频纹理恢复，从而形成“结构先行、纹理补偿、细节增强”的重建路径。最后，三路输出恢复到统一 token 粒度后进行拼接与投影融合：
...
为了进一步控制复杂度，本文拟采用不同的 head 分配策略，使高频分支承担主要细节恢复，中频和低频分支在更短序列上完成结构级交互；同时在高频与中频分支引入全局交互信息，在低频分支避免额外冗余叠加。该设计能够在不显著增加参数量的前提下，提升模型对复杂结构和高频细节的联合恢复能力。

综上所述，本文的研究方法以 CAPANet Block 为基本单元，将 DPR 的内容感知 token 组织能力、PFSA 的渐进聚焦连接筛选能力和 LMR 的低到高多级重建能力统一起来。DPR 解决“哪些 token 应该被组织到一起”的问题，PFSA 解决“哪些连接值得参与计算”的问题，LMR 解决“不同层次信息如何递进重建”的问题。三者协同作用，使模型能够在轻量级预算下更充分地利用跨区域相似纹理，减少无效注意力计算，并提高复杂场景中结构连续性和纹理细节恢复质量，为移动端和边缘设备场景下的高效图像超分辨率提供可行技术方案。
...
在训练设置方面，LR patch size 设为 64 × 64，batch size 设为 32，优化器采用 AdamW，betas = (0.9, 0.999)，weight_decay = 1e-4，初始学习率设为 5e-4，并采用 cosine decay 学习率衰减策略。主实验训练 500K iterations，消融实验统一训练 300K iterations；数据增强方式包括随机翻转、随机旋转和随机 RGB shuffle。默认结构配置采用 head_split = [2, 1, 1]、动态焦点率范围 [0.25, 0.75]、继承权重 beta = 0.5，并在高频与中频分支保留全局交互分支。
